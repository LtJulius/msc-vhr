...
struct alignas(16) PushConstantPayload {
	uint32_t primitives;
...
virtual_pipeline::~virtual_pipeline() noexcept {
...
vk::DescriptorSet virtual_pipeline::allocate_descriptor_set(
...
void virtual_pipeline::setup(const context& context_, vk::Device device_) {

	_device = device_;

	#ifdef _WIN32
	auto mono_shader_code = read_shader_file(L"./shader/virtual_monolithic_v1.spirv");
	#else
    auto mono_shader_code = read_shader_file(R"(./shader/virtual_monolithic_v1.spirv)");
	#endif
	...
	pool_sizes.emplace_back(vk::DescriptorPoolSize { vk::DescriptorType::eUniformBuffer, expected_sets * 4uL });
	pool_sizes.emplace_back(vk::DescriptorPoolSize { vk::DescriptorType::eStorageBuffer, expected_sets * 4uL });
	pool_sizes.emplace_back(vk::DescriptorPoolSize { vk::DescriptorType::eStorageImage, expected_sets * 4uL });
