...
virtual_pipeline_v3::virtual_pipeline_v3() {}

virtual_pipeline_v3::~virtual_pipeline_v3() noexcept {
...
vk::DescriptorSet virtual_pipeline_v3::allocate_descriptor_set(
...
void virtual_pipeline_v3::setup(const context& context_, vk::Device device_) {

	_device = device_;

	#ifdef _WIN32
	auto mono_shader_code = read_shader_file(L"./shader/virtual_monolithic_v3.spirv");
	#else
    auto mono_shader_code = read_shader_file(R"(./shader/virtual_monolithic_v3.spirv)");
	#endif
...
void virtual_pipeline_v3::record(
	const context& context_,
	const uint32_t idx_,
	const dataset& dataset_,
