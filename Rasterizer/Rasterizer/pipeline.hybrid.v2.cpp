...
hybrid_pipeline_v2::hybrid_pipeline_v2() {}

hybrid_pipeline_v2::~hybrid_pipeline_v2() noexcept {
...
void hybrid_pipeline_v2::ensure_intermediate(const context& context_, const uint32_t idx_, const dataset& dataset_) {
...
vk::DescriptorSet hybrid_pipeline_v2::allocate_analytic_descriptor_set(
...
vk::DescriptorSet hybrid_pipeline_v2::allocate_virtual_descriptor_set(
	const context& context_,
	const uint32_t idx_,
	const dataset& dataset_
) {
...
vk::DescriptorSet hybrid_pipeline_v2::allocate_native_descriptor_set(
	const context& context_,
	const uint32_t idx_,
	const dataset& dataset_
) {
...
void hybrid_pipeline_v2::setup(const context& context_, vk::Device device_) {
	...
	{
		#ifdef _WIN32
		auto mono_shader_code = read_shader_file(L"./shader/hybrid_compute_v2.spirv");
		#else
        auto mono_shader_code = read_shader_file(R"(./shader/hybrid_compute_v2.spirv)");
		#endif
	...
	{
		#ifdef _WIN32
		auto vertex_shader_code = read_shader_file(L"./shader/hybrid_vertex_v2.spirv");
		auto fragment_shader_code = read_shader_file(L"./shader/hybrid_fragment_v2.spirv");
		#else
        auto vertex_shader_code = read_shader_file(R"(./shader/hybrid_vertex_v2.spirv)");
        auto fragment_shader_code = read_shader_file(R"(./shader/hybrid_fragment_v2.spirv)");
		#endif
	...
	{
		#ifdef _WIN32
		auto mono_shader_code = read_shader_file(L"./shader/hybrid_analytic_v2.spirv");
		#else
        auto mono_shader_code = read_shader_file(R"(./shader/hybrid_analytic_v2.spirv)");
		#endif
...
void hybrid_pipeline_v2::record(
	const context& context_,
	const uint32_t idx_,
	const dataset& dataset_,
...

	{
		...
		if (_intermediate_indices.empty()) {
			rootCmdBuffer_.dispatchIndirect(
				_intermediate_draw[idx_].vkBuffer,
				offsetof(IntermediateDrawPayload, draw_virtual)
			);
