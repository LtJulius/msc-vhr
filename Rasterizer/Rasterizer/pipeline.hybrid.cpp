...
struct IntermediateIndexedDrawPayload {
	vk::DispatchIndirectCommand draw_virtual;
	vk::DrawIndexedIndirectCommand draw_native;
};
...
hybrid_pipeline::~hybrid_pipeline() noexcept {
	...
	_device.destroyShaderModule(_virtual_shader_module);

	_device.destroyDescriptorPool(_descriptor_pool);
	_device.destroyDescriptorSetLayout(_virtual_descriptor_layout);
	...
	for (auto&& buffer : _intermediate_indices) {
		_device.destroyBuffer(buffer.vkBuffer);
		_device.freeMemory(buffer.vkDeviceMemory);
	}
	_intermediate_indices.clear();
}

void hybrid_pipeline::ensure_intermediate(const context& context_, const uint32_t idx_, const dataset& dataset_) {
	...
	if (
		_intermediate_indices.size() > idx_ &&
		(_intermediate_indices[idx_].size / sizeof(uint32_t)) < (dataset_.vertices.size / sizeof(vertex))
	) {

		_device.destroyBuffer(std::exchange(_intermediate_indices[idx_].vkBuffer, nullptr));
		_device.freeMemory(std::exchange(_intermediate_indices[idx_].vkDeviceMemory, nullptr));
		_intermediate_indices[idx_].size = 0uLL;
		_intermediate_indices[idx_].offset = 0uLL;
	}
	...
	if (_intermediate_draw.size() <= idx_) {
		...
		const auto bci = vk::BufferCreateInfo {
			...
			vk::DeviceSize { sizeof(IntermediateIndexedDrawPayload) + sizeof(IntermediateMetaPayload) },
	...
	if (
		...
		const auto bci = vk::BufferCreateInfo {
			vk::BufferCreateFlags {},
			vk::DeviceSize { sizeof(uint32_t) * (dataset_.vertices.size / sizeof(vertex)) },
		...
		if (_intermediate_indices.size() > idx_) {
			_intermediate_indices[idx_] = std::move(intermediate);
		} else {
			_intermediate_indices.emplace_back(std::move(intermediate));
		}
	}
}

vk::DescriptorSet hybrid_pipeline::allocate_analytic_descriptor_set(
	...
	vk::DescriptorBufferInfo intermediate_index_buffer_info = {};
	...
	{
		intermediate_index_buffer_info = vk::DescriptorBufferInfo {
			_intermediate_indices[idx_].vkBuffer,
			_intermediate_indices[idx_].offset,
			_intermediate_indices[idx_].size
		};
		writes.emplace_back(
			set,
			2uL,
			...
			vk::DescriptorType::eStorageBuffer,
			nullptr,
			&intermediate_index_buffer_info,
			nullptr
		);
	}
...
vk::DescriptorSet hybrid_pipeline::allocate_virtual_descriptor_set(
	const context& context_,
	const uint32_t idx_,
	const dataset& dataset_
) {

	if (_virtual_descriptor_sets.size() <= idx_) {

		vk::DescriptorSetAllocateInfo dsai {
			_descriptor_pool,
			1uL,
			&_virtual_descriptor_layout,
			nullptr
		};
		...
		_virtual_descriptor_sets.emplace_back(set);
	}
	...
	auto& set = _virtual_descriptor_sets[idx_];
	std::vector<vk::WriteDescriptorSet> writes {};
...

	{
		intermediate_meta_buffer_info = vk::DescriptorBufferInfo {
			_intermediate_draw[idx_].vkBuffer,
			_intermediate_draw[idx_].offset + sizeof(IntermediateIndexedDrawPayload),
	...
	{
		vertex_buffer_info = vk::DescriptorBufferInfo {
			dataset_.vertices.vkBuffer,
			dataset_.vertices.offset,
			dataset_.vertices.size
		};
		writes.emplace_back(
			set,
			2uL,
...
vk::DescriptorSet hybrid_pipeline::allocate_native_descriptor_set(
	const context& context_,
	const uint32_t idx_,
	const dataset& dataset_
) {
...
void hybrid_pipeline::setup(const context& context_, vk::Device device_) {
	...
	{
		#ifdef _WIN32
		auto mono_shader_code = read_shader_file(L"./shader/hybrid_compute_v1.spirv");
		#else
        auto mono_shader_code = read_shader_file(R"(./shader/hybrid_compute_v1.spirv)");
		#endif
		...
		_virtual_shader_module = device_.createShaderModule(msmci);
		...
		_virtual_descriptor_layout = device_.createDescriptorSetLayout(dslci);
		layouts.emplace_back(_virtual_descriptor_layout);
		...
		_virtual_pipeline_layout = device_.createPipelineLayout(plci);

		cpci.flags = vk::PipelineCreateFlags {};
		cpci.layout = _virtual_pipeline_layout;
		...
		_virtual_pipeline = result.value;
	}
	...
	{
		#ifdef _WIN32
		auto vertex_shader_code = read_shader_file(L"./shader/hybrid_vertex_v1.spirv");
		auto fragment_shader_code = read_shader_file(L"./shader/hybrid_fragment_v1.spirv");
		#else
        auto vertex_shader_code = read_shader_file(R"(./shader/hybrid_vertex_v1.spirv)");
        auto fragment_shader_code = read_shader_file(R"(./shader/hybrid_fragment_v1.spirv)");
		#endif
	...
	{
		#ifdef _WIN32
		auto mono_shader_code = read_shader_file(L"./shader/hybrid_analytic_v1.spirv");
		#else
        auto mono_shader_code = read_shader_file(R"(./shader/hybrid_analytic_v1.spirv)");
		#endif
		...
		// Storage Buffer : Intermediate Index
		layout_bindings.emplace_back(
			2uL,
			vk::DescriptorType::eStorageBuffer,
			1uL,
...

	{
		...
		pool_sizes.emplace_back(vk::DescriptorType::eStorageBuffer, expected_sys_count * expected_sets * 5uL);
		pool_sizes.emplace_back(vk::DescriptorType::eStorageImage, expected_sys_count * expected_sets * 5uL);
		pool_sizes.emplace_back(vk::DescriptorType::eUniformBuffer, expected_sys_count * expected_sets * 5uL);
...
void hybrid_pipeline::record(
...

	{
		const auto indirect_buffer_barrier = std::vector<vk::BufferMemoryBarrier> {
			vk::BufferMemoryBarrier {
				...
				_intermediate_draw[idx_].vkBuffer,
				0uL,
				sizeof(IntermediateIndexedDrawPayload),
				nullptr
			},
			vk::BufferMemoryBarrier {
				vk::AccessFlagBits::eShaderWrite,
	...
	/**
	 * Dispatch Virtual Sub-Pipeline
	 */

	{
		rootCmdBuffer_.bindPipeline(vk::PipelineBindPoint::eCompute, _virtual_pipeline);

		auto descriptor_set = allocate_virtual_descriptor_set(context_, idx_, dataset_);
		rootCmdBuffer_.bindDescriptorSets(
			vk::PipelineBindPoint::eCompute,
			_virtual_pipeline_layout,
			0uL,
			1uL,
			&descriptor_set,
			0uL,
		...
		rootCmdBuffer_.dispatchIndirect(
			_intermediate_draw[idx_].vkBuffer,
			offsetof(IntermediateIndexedDrawPayload, draw_virtual)
		);
	}
...

	{
		...
		rootCmdBuffer_.drawIndexedIndirect(
			_intermediate_draw[idx_].vkBuffer,
			offsetof(IntermediateIndexedDrawPayload, draw_native),
			1uL,
