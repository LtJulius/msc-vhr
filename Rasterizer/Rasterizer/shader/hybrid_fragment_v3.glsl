...
layout(set = 0, binding = 2, r32ui) uniform restrict coherent uimage2D target_image;
...
void main() {
    ...
    const uint32_t location = uint32_t(gl_FragCoord.x) + uint32_t(round(gl_FragCoord.y) * intermediate.data.resolution.x);

    const uint64_t packed = (uint64_t(packF32(depth)) << 33) | uint64_t((in_vertex_index / 3) << 1);
    const uint64_t stored = atomicMin(depth_buffer.data[location], packed);
