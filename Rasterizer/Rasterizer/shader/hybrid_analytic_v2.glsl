...
layout(scalar, set = 0, binding = 2) writeonly buffer InterVertexData {
    Vertex data[];
} inter_vertices;
...
void main() {
    ...
    if ( threadId < consts.primitives ) {
        ...
        // Load Vertices
        const u32vec3 indices = u32vec3(threadId * 3u + 0u, threadId * 3u + 1u, threadId * 3u + 2u);
        ...
        if (area < 4) {
            ...
            inter_vertices.data[store_index - 2].px = vp0.x;
            inter_vertices.data[store_index - 1].px = vp1.x;
            inter_vertices.data[store_index - 0].px = vp2.x;

            inter_vertices.data[store_index - 2].py = vp0.y;
            inter_vertices.data[store_index - 1].py = vp1.y;
            inter_vertices.data[store_index - 0].py = vp2.y;

            inter_vertices.data[store_index - 2].pz = vp0.z;
            inter_vertices.data[store_index - 1].pz = vp1.z;
            inter_vertices.data[store_index - 0].pz = vp2.z;

            inter_vertices.data[store_index - 2].pw = vp0.w;
            inter_vertices.data[store_index - 1].pw = vp1.w;
            inter_vertices.data[store_index - 0].pw = vp2.w;

            inter_vertices.data[store_index - 2].c = vertices.data[indices.x].c;
            inter_vertices.data[store_index - 1].c = vertices.data[indices.y].c;
            inter_vertices.data[store_index - 0].c = vertices.data[indices.z].c;

        } else {

            const uint inter_base_index = atomicAdd(intermediate.inter_draw.vertexCount, 3);
            indirect_draw_vertices = true;

            //

            inter_vertices.data[inter_base_index + 0].px = pp0.x;
            inter_vertices.data[inter_base_index + 0].py = pp0.y;
            inter_vertices.data[inter_base_index + 0].pz = pp0.z;
            inter_vertices.data[inter_base_index + 0].pw = pp0.w;

            inter_vertices.data[inter_base_index + 1].px = pp1.x;
            inter_vertices.data[inter_base_index + 1].py = pp1.y;
            inter_vertices.data[inter_base_index + 1].pz = pp1.z;
            inter_vertices.data[inter_base_index + 1].pw = pp1.w;

            inter_vertices.data[inter_base_index + 2].px = pp2.x;
            inter_vertices.data[inter_base_index + 2].py = pp2.y;
            inter_vertices.data[inter_base_index + 2].pz = pp2.z;
            inter_vertices.data[inter_base_index + 2].pw = pp2.w;

            inter_vertices.data[inter_base_index + 0].c = vertices.data[indices.x].c;
            inter_vertices.data[inter_base_index + 1].c = vertices.data[indices.y].c;
            inter_vertices.data[inter_base_index + 2].c = vertices.data[indices.z].c;
        }
    ...
    if (gl_LocalInvocationIndex == 0u && indirect_dispatch_primitives) {
        intermediate.inter_dispatch.z = 1;
        intermediate.inter_dispatch.y = 1;
